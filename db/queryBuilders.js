...
class planets {
  static async getAll() {
    return db.select().table("planets");
  }
  static async getByCode(code) {
    return db.first().table("planets").where("code", code);
  }
}

class spaceCenters {
  static async getBy(args) {
    // parameter is an obj in which we expect an id/uid
  ...
  static async getbyUID(uid) {
    return db("space_centers").first().where("uid", uid);
  }
  ...
  static async getCount() {
    return db("space_centers").count("id").first();
  }
}

class flights {
  static async getById(id) {
    return db.first().table("flights").where("id", id);
  }
  static async getAllPaginated(filters, page, pageSize) {
    ...
    return db
      ...
      .modify(function (queryBuilder) {
        if (filters.from) {
          // these conditionals may need to be typed
  ...
  static async getCount() {
    return db("flights").count("id").first();
  }
  static async getByCode(code) {
    return db.first().table("flights").where("code", code);
  }
...
class bookings {
  static async getById(id) {
    return db.first().table("bookings").where("id", id);
  }
  ...
  static async getCount() {
    return db("bookings").count("id").first();
  }
...
export { planets, spaceCenters, flights, bookings };
